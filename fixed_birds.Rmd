...
```{r}
birds_df <- left_join(birds_1, taxalist_1, by = "species_id")
```
...
```{r}
# can't use a semi-join because park_name isn't shared, so it would be omitted
...
ggplot(data = selected_parks,
       aes(y = common_name, x = total_count)) +
  geom_col() +
